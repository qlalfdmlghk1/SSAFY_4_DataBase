...
-- 난수
select rand(), floor(rand() * 45 + 1),floor(rand() * 45 + 1),floor(rand() * 45 + 1),floor(rand() * 45 + 1),floor(rand() * 45 + 1),floor(rand() * 45 + 1);
...
select '123-456', substr('123-465',1, instr('123-456', '-') - 1), substr('123-456', instr('123-456', '-') + 1);
...
-- hello  fy !!!
select 'hello ssafy !!!', concat(left('hello ssafy !!!', 5), '...');


-- 3자리마다 콤마(,) & 소수점 2자리까지
select format(123456789.987, 2);


-- 공백제거, 문자기준 trim
select rtrim('     aa      '),ltrim('     aa      '),trim('     aa      ');
...
-- 날짜 세부 반환 함수 : 
select year(now()), month(now()), month('2024-10-11'), dayofmonth(now()), dayofweek(now());
...
-- ------------------- 논리 관련 함수 ----------------------
-- 논리함수 : 크다  작다  3  b  a
select if(3>2, '크다', '작다'), ifnull(null,'a'), ifnull('b','a'), nullif('a','a'), nullif('a','a');

-- ------------------- 집계 함수 ----------------------
-- employees table에서 사원의 총수, 급여의 합, 급여의 평균, 최고급여, 최저급여
select count(employee_id) 사원수, sum(salary) 급여총합, avg(salary) 급여평균, max(salary) 최대급여
from employees;
